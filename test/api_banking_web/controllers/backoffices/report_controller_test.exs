defmodule ApiBankingWeb.Backoffices.ReportControllerTest do
  ...
  test "report/2 return the total transitioned on system", %{conn: conn} do
    ...
    req_conn =
      ...
      |> get(
        Routes.backoffice_report_path(conn, :report, %{"date" => "2018-01-01", "period" => "year"})
      )
    ...
    req_conn =
      ...
      |> put_req_header("authorization", "bearer " <> token)
      |> get(
        Routes.backoffice_report_path(conn, :report, %{"date" => "2018-11-06", "period" => "day"})
      )
    ...
    req_conn =
      ...
      |> put_req_header("authorization", "bearer " <> token)
      |> get(
        Routes.backoffice_report_path(conn, :report, %{"date" => "2019-01-01", "period" => "year"})
      )
    ...
    req_conn =
      ...
      |> put_req_header("authorization", "bearer " <> token)
      |> get(
        Routes.backoffice_report_path(conn, :report, %{
          "date" => "2018-06-10",
          "period" => "month"
        })
      )
    ...
    assert Decimal.cast(response["total"]) ==
             Decimal.add(Decimal.add(amount_2018, amount_2018_1), amount_2019)
  end
